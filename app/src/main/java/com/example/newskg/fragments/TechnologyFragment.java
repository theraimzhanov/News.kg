...
public class TechnologyFragment extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        recyclerTechnology = getActivity().findViewById(R.id.recyclerTechnology);
        list = new ArrayList<>();
        newsAdapter = new NewsAdapter(list,getActivity());
        Date date = new Date();
        String data = String.valueOf(date.getTime());
        recyclerTechnology.setAdapter(newsAdapter);
        recyclerTechnology.setLayoutManager(new LinearLayoutManager(getActivity()));
        RetrofitBuilder.getService().getAllNews("technology",data,"popularity","fdc952d04aa64ed4be22bd6c1e870552").enqueue(new Callback<News>() {
            @Override
